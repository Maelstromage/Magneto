        ...
        if lowest[2] < self.heal_other_threshold:
            ...
            if self.q_type == 'heal':
                ...
                if self.q_target == 'unit':
                    action_hotbar('q','allies')
                if self.q_target == 'allies none':
                    action_hotbar('q','allies none') #no reason not to just pass variable q_target >.<
            if self.w_type == 'heal':
                if self.w_target == 'unit':
                    action_hotbar('w','allies')
                if self.w_target == 'allies none':
                    action_hotbar('w','allies none')
            if self.e_type == 'heal':
                if self.e_target == 'unit':
                    action_hotbar('e','allies')
                if self.e_target == 'allies none':
                    action_hotbar('e','allies none')
            if self.r_type == 'heal':
                if self.r_target == 'unit':
                    action_hotbar('r','allies')
                if self.r_target == 'allies none':
                    action_hotbar('r','allies none')
            if self.d_type == 'heal':
                if self.d_target == 'unit':
                    action_hotbar('d','allies')
                if self.d_target == 'allies none':
                    action_hotbar('d','allies none')
def action_attack():
    ...
    if self.q_type == 'attack':
        action_hotbar('q','enemy')
    if self.w_type == 'attack':
        action_hotbar('w','enemy')
    if self.e_type == 'attack':
        action_hotbar('e','enemy')
    if self.r_type == 'attack':
        action_hotbar('r','enemy')
    if self.d_type == 'attack':
        action_hotbar('d','enemy')
    ...
    if gamemode == 1:
        ...
        if hero_1 == 'suicide':
            if rotationtime + 5 < etime:
                ...
                if redcore != None:
                    _thread.start_new_thread(action_attack_core,(redcore.left, redcore.top)) 
